...
import { View, Text, StyleSheet, TextInput, FlatList, TouchableOpacity, ActivityIndicator, Keyboard, KeyboardAvoidingView, Platform } from 'react-native';
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.searchContainer}>
        <View style={styles.inputWrapper}>
          ...
          <TextInput ref={inputRef} style={styles.input} value={query} onChangeText={handleQueryChange} placeholder="Buscar por nombre, SKU, editorial..." placeholderTextColor={Colors.textMuted} returnKeyType="search" autoCorrect={false} autoCapitalize="none" />
          {query.length > 0 && <TouchableOpacity onPress={handleClear} style={styles.clearButton}><Text style={styles.clearText}>✕</Text></TouchableOpacity>}
        </View>
        <FlatList horizontal data={CATEGORIES} keyExtractor={item => item.key} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryList}
          renderItem={({ item }) => (
            <TouchableOpacity style={[styles.categoryChip, activeCategory === item.key && styles.categoryChipActive]} onPress={() => handleCategoryChange(item.key)}>
              <Text style={[styles.categoryChipText, activeCategory === item.key && styles.categoryChipTextActive]}>{item.label}</Text>
            </TouchableOpacity>
          )}
        />
      </View>
      {loading ? (
        <View style={styles.centerState}><ActivityIndicator color={Colors.primary} size="large" /><Text style={styles.loadingText}>Buscando...</Text></View>
      ) : !searched ? (
        <View style={styles.centerState}>
          <Text style={styles.emptyIcon}>🗂</Text>
          <Text style={styles.emptyTitle}>Consulta de Productos</Text>
          <Text style={styles.emptySubtitle}>Escribe el nombre, SKU o editorial para buscar.</Text>
        </View>
      ) : results.length === 0 ? (
        <View style={styles.centerState}>
          <Text style={styles.emptyIcon}>😕</Text>
          <Text style={styles.emptyTitle}>Sin resultados</Text>
          <Text style={styles.emptySubtitle}>No se encontró ningún producto con "{query}".</Text>
        </View>
      ) : (
        <FlatList data={results} keyExtractor={item => item.id.toString()}
          renderItem={({ item }) => <ProductCard product={item} onPress={() => { Keyboard.dismiss(); setSelectedProduct(item); }} />}
          contentContainerStyle={styles.listContent} keyboardShouldPersistTaps="handled"
          ListHeaderComponent={<Text style={styles.resultCount}>{results.length} producto{results.length !== 1 ? 's' : ''} encontrado{results.length !== 1 ? 's' : ''}</Text>}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  searchContainer: { backgroundColor: Colors.surface, paddingTop: Spacing.sm, borderBottomWidth: 1, borderBottomColor: Colors.border },
